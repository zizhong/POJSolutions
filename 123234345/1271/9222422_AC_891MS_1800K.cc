...
public:
  ...
  point pt[maxpn];
	...
	polygon& halfPanelCross(pVector pv[], int ln)
	{
		int i, tn;
		n = 0;
		std::sort(pv, pv + ln, hpc_cmp);
		
		for(i = tn = 1; i < ln; i ++)
			if(fabs(pv[i].ang - pv[i - 1].ang) > eps)
				pv[tn ++] = pv[i];
		ln = tn;
		int bot = 0, top = 1;
		dequeue[0] = pv[0];
		...
		for(i = 2; i < ln; i ++)
		{
			if(dequeue[top].parallel(dequeue[top - 1]) ||
				...
				return (*this);
			while(bot < top &&
				point::xmult(dequeue[top].crossLPt(dequeue[top - 1]), pv[i].e, pv[i].s) > eps)
				top --;
			while(bot < top &&
				point::xmult(dequeue[bot].crossLPt(dequeue[bot + 1]), pv[i].e, pv[i].s) > eps)
				bot ++;
			dequeue[++ top] = pv[i];
		}
...
int main()
{
    ...
    while(scanf("%d%d%d",&n,&k,&h),n)
    {
         ...
         for(int j=0;j<n;j++) pv2[j]=pVector(pv1[j].s+poff[j],pv1[j].e+poff[j]);
         ans=0.0;
         ...
         for(int i=0;i<cnt[k];i++)
         {
              ...
              polygon po1=po1.halfPanelCross(pv,n);
