...
#include<cstring>
char s[10][16];
char tmp[10][16],cnt[10][16],cc;

bool isOk(char c)
{
     return c=='R'||c=='G'||c=='B';
}
void disp(char g[10][16])
{
    ...
    for(int tt=1;tt<=T;tt++)
    {
         ...
         while(1)
         {
              ...
              {   
               ...
               //if(cnt[x][y]==37) disp(s);
              //right->left
              int line[15],nline=0;
              for(int i=0;i<15;i++)  if(s[0][i])
                  line[nline++]=i;
              //if(cnt[x][y]==37) {for(int i=0;i<nline;i++) printf("%d",line[i]),putchar(' ');puts("-_-");}
