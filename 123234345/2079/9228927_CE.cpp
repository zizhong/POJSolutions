...
int main()
{
    while(scanf("%d",&po0.n),po0.n>0)
    {
         ...
         for(int i=0;i<po1.n;i++)
         {
              ...
              while(k!=i&&point::xmult(po1.pt[j],po1.pt[k],po1.pt[i]) +eps<
                    point::xmult(po1.pt[j],po1.pt[(k+1)%po1.n],po1.pt[i]))
                          k=(k+1)%po1.n;
              double tmp=0.5*point::xmult(po1.pt[j],po1.pt[k],po1.pt[i]);
              if(tmp>ans) ans=tmp;
              if(k==i) continue;
              while(j!=k&&point::xmult(po1.pt[j],po1.pt[k],po1.pt[i]) +eps<
                    point::xmult(po1.pt[(j+1)%po1.n],po1.pt[k],po1.pt[i]))
                          j=(j+1)%po1.n;
              tmp=0.5*point::xmult(po1.pt[j],po1.pt[k],po1.pt[i]);
              if(tmp>ans) ans=tmp;
         }
         if(ans<=eps) puts("-1");
         else printf("%.2f\n",ans+eps);
