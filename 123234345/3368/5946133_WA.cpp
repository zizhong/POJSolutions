...
int query(int l,int r,int num,int &p)
{
    ...
    if(segt[num].l==l&&segt[num].r==r) return p=num,segt[num].n;
    int mid=(segt[num].l+segt[num].r)>>1;
    if(r<=mid) return query(l,r,2*num,p);
    if(mid<=l) return query(l,r,2*num+1,p);
    int num1,num2;
    int n1=query(l,mid,2*num,num1),n2=query(mid,r,2*num+1,num2);
    int max=_max(n1,n2);
    if(d[mid-1]==d[mid]) max=_max(max,segt[num1].nr+segt[num2].nl);
    return max;
}
int main()
{
