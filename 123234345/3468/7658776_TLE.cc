...
{
     if(seg[idx].id&&l==seg[idx].l&&seg[idx].r==r)
     {
           seg[idx].v+=v;
           return ;
     }
     ...
     {
          seg[idx].id=0;
          seg[2*idx].id=seg[2*idx+1].id=1;
          seg[2*idx].v=seg[2*idx+1].v=seg[idx].v;
     }
     int mid=seg[idx].l+seg[idx].r>>1;
     if(r<=mid) modify(l,r,v,2*idx);
     else if(mid+1<=l) modify(l,r,v,2*idx+1);
     else modify(l,mid,v,2*idx),modify(mid+1,r,v,2*idx+1);
}
__int64 sumseg(int l,int r,int idx)
{
        if(seg[idx].id) 
            return (__int64)seg[idx].v*(r-l+1);
        int mid=seg[idx].l+seg[idx].r>>1;
