                ...
                for(int m=1;m<=noZx[tmp[4][0]][0];m++)
                {
                    ...
                    for(int n=1;n<=xnum[tmp[4][4]][0];n++)
                    {
                        ...
                        for(int d=0;d<10;d++)
                        {
                            ...
                            if(p[ tmp[0][1]*ten[4]+tmp[1][1]*ten[3]+tmp[2][1]*ten[2]+tmp[3][1]*ten[1]+tmp[4][1] ] ||
                               p[ tmp[0][2]*ten[4]+tmp[1][2]*ten[3]+tmp[2][2]*ten[2]+tmp[3][2]*ten[1]+tmp[4][2] ] || 
                               ...
                               p[ tmp[2][0]*ten[4]+tmp[2][1]*ten[3]+tmp[2][2]*ten[2]+tmp[2][3]*ten[1]+tmp[2][4] ] ||
                               p[ tmp[3][0]*ten[4]+tmp[3][1]*ten[3]+tmp[3][2]*ten[2]+tmp[3][3]*ten[1]+tmp[3][4] ] ||
                               p[ tmp[4][0]*ten[4]+tmp[3][1]*ten[3]+tmp[2][2]*ten[2]+tmp[1][3]*ten[1]+tmp[0][4] ] )
                               continue;
