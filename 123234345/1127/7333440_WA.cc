...
int dis2(Point a,Point b){return (a.x-b.x)*(a.x-b.x)+(a.y-b.y)*(a.y-b.y);}
int main()
{
    int n;
    while(scanf("%d",&n),n)
         ...
         for(int i=1;i<=n;i++)
             for(int j=1;j<=n;j++)
             {
                if(segcross(p[i][0],p[i][1],p[j][0],p[j][1]))
                    g[i][j]=1;
                if(dis2(p[i][0],p[j][1])+dis2(p[i][1],p[j][1])==dis2(p[i][0],p[i][1])) g[i][j]=1;
                if(dis2(p[i][0],p[j][0])+dis2(p[i][1],p[j][0])==dis2(p[i][0],p[i][1])) g[i][j]=1;
                if(dis2(p[i][0],p[j][1])+dis2(p[i][0],p[j][0])==dis2(p[j][0],p[j][1])) g[i][j]=1;
                if(dis2(p[i][1],p[j][0])+dis2(p[i][1],p[j][1])==dis2(p[j][0],p[j][1])) g[i][j]=1;
             }
         for(int k=1;k<=n;k++)
              for(int i=1;i<=n;i++)
                  for(int j=1;j<=n;j++)
