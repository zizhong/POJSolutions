...
int ok(double x)
{
     for(int i=0;i<n;i++)
     {
         poff[i]=(pt[(i+1)%n]-pt[i]).toLeft().setLength(x);
         //printf("%.3f %.3f\n",poff[i].x,poff[i].y);
         pv[i]=pVector(pt[i]+poff[i],pt[(i+1)%n]+poff[i]);
     }
     polygon poly1;
     poly1=poly1.halfPanelCross(pv,n);
     return poly1.n;
}
int main()
{
