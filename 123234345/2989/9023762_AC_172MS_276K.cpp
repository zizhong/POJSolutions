		  ...
		  for(int i=0;i<n&&cnt<=1000;i++)
		  {
			   ...
			   for(int j=0;j<cnt;j++)
			   {
					 ...
					 else
					 {
					     ...
					     if(c2==cliq2[j]) cliq2[j].insert(i),flag=1,flag2[j]=1;                        
					     else if(!c2.empty()) c2.insert(i),cliq2[cnt+inc]=c2,flag=1,inc++;
					 }
			   }
			   if(!flag) cliq2[cnt++]=cur;
		       int tmp[2048]={0};
		       for(int i=0;i<cnt+inc;i++) if((i<cnt&&flag2[i])||(i>=cnt&&tmp[i]==0))
		           for(int j=cnt;j<inc+cnt;j++) if(i!=j)
                       if(cliq2[i].intersect(cliq2[j])==cliq2[j]) tmp[j]=1;
               int tot=0;
               for(int i=0;i<cnt+inc;i++) if(!tmp[i])  cliq2[tot++]=cliq2[i];
               cnt=tot;
